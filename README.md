...
- Results stay `pending_officer_review` until reviewed; the verification route does not change immigration cases, watchlist screening, or identity dispositions. A linked border event is a factual entry/exit record and is not an admission or denial decision.
...
The single configured operator token is a prototype authentication boundary for passport verification, review, and linked border-event endpoints, not JANUS/RBAC/MFA integration. Earlier Group 1 APIs remain unauthenticated and the static workspace has no new passport capture controls. Do not expose this prototype to real traveler data. The application applies the additive border-event schema change at startup for existing databases, but production deployment still needs managed migrations/release controls, system-wide authenticated document/capture provenance, authenticated operators, TLS, provider/device integration, and retention/audit policy. These are not claimed as completed by this change.
